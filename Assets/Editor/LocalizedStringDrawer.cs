...
public class LocalizedStringDrawer : PropertyDrawer
{
    ...
    public override float GetPropertyHeight(SerializedProperty proprety, GUIContent label)
    {
        if (dropdown) { return height + 25; }
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        Texture storeIcon = (Texture)Resources.Load("Localization/add");
        ...
        if (dropdown)
        {
            var value = LocalizationSystem.GetLocalisedValues(key.stringValue);
